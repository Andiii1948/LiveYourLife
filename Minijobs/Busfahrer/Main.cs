...
using LiveYourLife.Utils;

namespace LiveYourLife.Minijobs.Busfahrer
{
    public class Main : IScript
    {
        ...
        public static void Initialize()
        {
            ...
            var markerData = new Server_Markers { type = 39, posX = Utils.Constants.Positions.Minijob_Busdriver_VehOutPos.X, posY = Utils.Constants.Positions.Minijob_Busdriver_VehOutPos.Y, posZ = Utils.Constants.Positions.Minijob_Busdriver_VehOutPos.Z + 1, alpha = 150, bobUpAndDown = true, scaleX = 1, scaleY = 1, scaleZ = 1, red = 46, green = 133, blue = 232 };
        ...
        private static async void PlayerExitVehicle(IVehicle vehicle, IPlayer player, byte seat)
        {
            try
            {
                ...
                if (!vehicle.Position.IsInRange(Utils.Constants.Positions.Minijob_Busdriver_VehOutPos, 8f)) return;
            ...
            try
            {
                ...
                if (client.GetPlayerCurrentMinijobStep() == "DRIVE_TO_NEXT_STATION" && state && client.IsInVehicle)
                {
                    ...
                    else if ((int)client.GetPlayerCurrentMinijobActionCount() >= maxSpots)
                    {
                        ...
                        client.EmitLocked("Client:Minijob:CreateJobMarker", "Minijob: Busabgabe", 3, 515, 30, Utils.Constants.Positions.Minijob_Busdriver_VehOutPos.X, Constants.Positions.Minijob_Busdriver_VehOutPos.Y, Constants.Positions.Minijob_Busdriver_VehOutPos.Z, false);
                        return;
                    }
                }
            }
        ...
        public async Task StartMiniJob(IPlayer player, int routeId)
        {
            try
            {
                ...
                {
                    if (veh == null || !veh.Exists) continue;
                    if (veh.Position.IsInRange(Utils.Constants.Positions.Minijob_Busdriver_VehOutPos, 8f)) { HUDHandler.SendNotification(player, 3, 5000, "Der Ausparkpunkt ist blockiert."); return; }
                }
                ServerVehicles.CreateVehicle(Model.GetRouteVehicleHash(routeId), charId, 2, 0, false, 0, Utils.Constants.Positions.Minijob_Busdriver_VehOutPos, Utils.Constants.Positions.Minijob_Busdriver_VehOutRot, $"BUS-{charId}", 132, 132);
